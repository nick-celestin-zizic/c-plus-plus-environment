...
*           If no default font loaded, DrawSomeTextEx() and MeasureTextEx() are required.
*
...
#include "rlgl.h"           // OpenGL abstraction layer to OpenGL 1.1, 2.1, 3.3+ or ES2 -> Only DrawSomeTextPro()
...
#endif
    {
        Image image = LoadAnImage(fileName);
...
Font LoadFontFromMemory(const char *fileType, const unsigned char *fileData, int dataSize, int fontSize, int *codepoints, int codepointCount)
{
        ...
        TextIsEqual(fileExtLower, ".otf"))
    {
        ...
        if (font.glyphs != NULL)
        {
            ...
            // Update glyphs[i].image to use alpha, required to be used on ImageDrawSomeText()
            for (int i = 0; i < font.glyphCount; i++)
            {
...
void DrawFPS(int posX, int posY)
{
    ...
    DrawSomeText(TextFormat("%2i FPS", fps), posX, posY, 20, color);
}

// Draw text (using default font)
// NOTE: fontSize work like in any drawing program but if fontSize is lower than font-base-size, then font-base-size is used
// NOTE: chars spacing is proportional to fontSize
void DrawSomeText(const char *text, int posX, int posY, int fontSize, Color color)
{
    ...
    if (GetFontDefault().texture.id != 0)
    {
        ...
        DrawSomeTextEx(GetFontDefault(), text, position, (float)fontSize, (float)spacing, color);
    }
}

// Draw text using Font
// NOTE: chars spacing is NOT proportional to fontSize
void DrawSomeTextEx(Font font, const char *text, Vector2 position, float fontSize, float spacing, Color tint)
{
...
void DrawTextPro(Font font, const char *text, Vector2 position, Vector2 origin, float rotation, float fontSize, float spacing, Color tint)
{
    rlPushMatrix();
        ...
        DrawSomeTextEx(font, text, (Vector2){ 0.0f, 0.0f }, fontSize, spacing, tint);
...
static Font LoadBMFont(const char *fileName)
{
    ...
    Image imFont = LoadAnImage(imPath);
